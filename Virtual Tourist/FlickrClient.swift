...
class FlickrClient {
    ...
    func taskForGetMethod(_ resource: String, parameters: [String: Any], completionHandler: @escaping (_ results: AnyObject?, _ error: NSError?) -> Void) -> URLSessionDataTask {
        
        // Set Parameters
        var parameters = parameters as [String: AnyObject]
        
        // Build URL and configure request
        let request = URLRequest(url: URLFromParameters(parameters, withPathExtension: resource))
        ...
        let task = session.dataTask(with: request, completionHandler: { (data, response, error) in
            ...
            if let error = error {
                sendError(1, errorString: error.localizedDescription)
                return
            }
