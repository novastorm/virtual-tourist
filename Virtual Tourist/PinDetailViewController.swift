...
class PinDetailViewController: UIViewController {
    ...
    lazy var fetchedResultsController: NSFetchedResultsController<Photo> = {
        ...
        let fetchedResultsController = NSFetchedResultsController<Photo>(fetchRequest: fetchRequest, managedObjectContext: self.sharedMainContext, sectionNameKeyPath: nil, cacheName: nil)
    ...
    func getPhotos() {
        ...
        _ = FlickrClient.sharedInstance.searchByLocation(latitude: lat, longitude: lon) { (results, error) in
            if let error = error {
                print(error)
                return
            }
            ...
            _ = FlickrClient.sharedInstance.searchByLocation(latitude: lat, longitude: lon, page: randomPage) { (results, error) in
...
extension PinDetailViewController: NSFetchedResultsControllerDelegate {
    ...
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
        ...
        switch type {
        case .insert:
            let newIndexPathAdjusted = IndexPath(item: newIndexPath!.item + numberOfStaticCells, section: 0)
